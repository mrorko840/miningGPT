...
class ProfileController extends Controller
{
    ...
    {   
        ...
        $earn['total'] = Transaction::where('user_id', auth()->user()->id)->where('remark', 'mining')->sum('amount');
        
        $earn['today'] = Transaction::where('user_id', auth()->user()->id)
                        ->where('remark', 'mining')
                        ->whereDate('created_at', Carbon::today())
                        ->sum('amount');
        return view($this->activeTemplate. 'user.display_profile', $data, compact(
            'pageTitle','user','widget', 'earn',
